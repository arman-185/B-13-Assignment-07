...
const FriendsDetails = () => {
    ...
    if (!friend) return <p className='text-center font-bold text-4xl mt-10'>Friend not found.</p>;
        ...
        <div className='bg-gray-100 flex items-center justify-center p-8'>
            <div className='bg-white border border-gray-300 rounded-xl flex w-full max-w-6xl'>
                ...
                <div className='w-56 border-r  border-gray-300 p-6 flex flex-col items-center gap-2'>
                    ...
                    <h2 className='font-bold text-gray-800'>{friend.name}</h2>
                    <span className={`text-xs px-3 py-1 rounded-full font-semibold ${statusStyle(friend.status)}`}>{statusLabel(friend.status)}</span>
                    <div className='flex flex-wrap justify-center gap-1'>
                        {friend.tags.map(tag => (
                            <span key={tag} className='bg-green-100 text-green-700 text-xs px-2 py-1 rounded-full capitalize'>{tag}</span>
                        ))}
                    </div>
                    <p className='text-gray-400 text-xs italic text-center'>"{friend.bio}"</p>
                ...
                <div className='flex-1 p-6 flex flex-col gap-4'>
                    ...
                    <div className='grid grid-cols-3 gap-3'>
                        {[
                            ...
                            { label: 'Goal (Days)', value: friend.goal_days },
                    ...
                    <div className='border border-gray-200 rounded-lg p-4'>
                        ...
                        <p className='text-gray-500 text-sm mt-1'>Connect every <span className='font-bold text-gray-800'>{friend.goal_days} days</span></p>
                    </div>
                    ...
                    <div className='border border-gray-200 rounded-lg p-4'>
                        ...
                        <div className='grid grid-cols-3 gap-3'>
                            ...
                            <button className='btn btn-sm btn-ghost border border-gray-200 flex flex-col h-14 gap-1'>
                                <Phone size={18} />
